...
def get_bist_symbols_from_finnhub():
    FINNHUB_API_KEY = "d15c2k9r01qntkapah2gd15c2k9r01qntkapah30"
    url = f"https://finnhub.io/api/v1/stock/symbol?exchange=BIST&token={FINNHUB_API_KEY}"
    ...
    try:
        ...
        if not isinstance(data, list):
            raise ValueError("Finnhub sembol listesi beklenmedik formatta: " + str(data))

        symbols = {
            item.get('displaySymbol', item.get('symbol')): item['symbol']
            for item in data
            if isinstance(item, dict) and 'symbol' in item and isinstance(item['symbol'], str) and item['symbol'].startswith("BIST:")
        }

        if not symbols:
            raise ValueError("Sembol listesi boş döndü. API limiti dolmuş olabilir.")

        return symbols

    except Exception as e:
        print("Finnhub sembol verisi alınamadı:", e)
        return {
            'ASELS': 'BIST:ASELS',
            'THYAO': 'BIST:THYAO',
            'KRDMD': 'BIST:KRDMD',
...
def get_stock_data(stock_symbol):
    ...
    url = f"https://finnhub.io/api/v1/stock/candle?symbol={symbol}&resolution={resolution}&from={int(start_date.timestamp())}&to={int(end_date.timestamp())}&token={FINNHUB_API_KEY}"
    response = requests.get(url)
    data = response.json()

    if "s" not in data or data.get("s") != "ok":
        raise ValueError(f"Finnhub API geçersiz yanıt: {data} - Sembol: {symbol}")
